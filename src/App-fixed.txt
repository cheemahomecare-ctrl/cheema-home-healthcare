...
import { listenToCollection, addToCollection, updateInCollection, deleteFromCollection, auth, signInWithEmailAndPassword, sendPasswordResetEmail, onAuthStateChanged, signOut } from "./firebase.js";
  ...
  return (
    ...
    >
      <div onClick={() => setPage("home")} style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
        <img
          src="/logo.png"
          alt="Cheema Home Healthcare logo"
          style={{
            width: 40,
            height: 40,
            borderRadius: 9,
...
function AdminLogin({ onSuccess }) {
  ...
  return (
    <main style={{ maxWidth: 360, margin: "60px auto", padding: "0 24px" }}>
      <div style={{ background: "#FFFFFF", border: "1.5px solid #E2D9C8", borderRadius: 14, padding: "32px 28px" }}>
        <h3 style={{ fontFamily: "'Fraunces', serif", color: "#14463D", margin: "0 0 6px", textAlign: "center" }}>
          Staff login
        </h3>
        <p style={{ fontSize: 13.5, color: "#8A8275", margin: "0 0 18px", textAlign: "center" }}>
        ...
        <form onSubmit={login}>
          ...
          <input
            ...
            style={{ ...inputStyle, marginBottom: 10 }}
          />
          {err && <p style={{ color: "#C5704B", fontSize: 13, marginBottom: 10 }}>{err}</p>}
          {info && <p style={{ color: "#6E8F73", fontSize: 13, marginBottom: 10 }}>{info}</p>}
          <button type="submit" disabled={busy} style={{ ...solidBtn, width: "100%", opacity: busy ? 0.7 : 1 }}>
        ...
        <button
          ...
          disabled={busy}
          style={{ ...ghostBtn, width: "100%", marginTop: 10, border: "none", textAlign: "center" }}
        >
          Forgot password?
        </button>
      </div>
...
function AdminDashboard({ bookings, loading, error, updateBooking, nurses, addNurse, removeNurse, onExit }) {
  ...
  const nurseNotifyText = (b) =>
    `Hello ${b.assignedNurseName}, you have been assigned a new home visit by Cheema Home Healthcare.\nPatient: ${b.name}\nService: ${NURSING_SERVICES.find((s) => s.id === b.service)?.label}\nDate: ${b.date} at ${b.time}\nAddress: ${b.address}\nPatient phone: ${b.phone}\n${b.notes ? "Notes: " + b.notes + "\n" : ""}Please confirm your availability for this visit.`;

  const assignNurse = (b, nurseId) => {
    const nurse = nurses.find((n) => n.id === nurseId);
    updateBooking(b.id, {
      assignedNurseId: nurse ? nurse.id : "",
      assignedNurseName: nurse ? nurse.name : "",
      assignedNursePhone: nurse ? nurse.phone : "",
    });
  };
        ...
        {filtered.map((b) => (
          <div key={b.id} style={{ background: "#FFFFFF", border: "1.5px solid #E2D9C8", borderRadius: 12, padding: "16px 18px" }}>
            ...
            <div style={{ marginTop: 12, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <span style={{ fontSize: 12.5, color: "#6E8F73", fontWeight: 600 }}>Assign nurse:</span>
              <select
                value={b.assignedNurseId || ""}
                onChange={(e) => assignNurse(b, e.target.value)}
                style={{ ...inputStyle, padding: "6px 10px", fontSize: 12.5, width: "auto" }}
              >
              ...
              {b.assignedNursePhone && (
                <WhatsAppButton
                  phone={formatPhoneForWa(b.assignedNursePhone)}
                  text={nurseNotifyText(b)}
                  style={{ padding: "6px 12px", fontSize: 12.5, background: "#14463D" }}
                >
                  Notify nurse
                </WhatsAppButton>
              )}
...
export default function CheemaNursingApp() {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#FAF7F0", minHeight: "100vh", color: "#1F2A24" }}>
      ...
      {page === "admin" &&
        (!authChecked ? (
          <main style={{ maxWidth: 360, margin: "60px auto", padding: "0 24px", textAlign: "center", color: "#8A8275" }}>
            Checking login…
          </main>
        ) : adminAuthed ? (
          <AdminDashboard
